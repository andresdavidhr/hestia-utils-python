...
def run_sql_oracle(conn_id, query, output_csv=None):
    logger = get_logger()
    ...
    with open(CONFIG_PATH, 'r') as f:
        config = yaml.safe_load(f)['oracle'][conn_id]
    ...
    logger.info(f"Iniciando consulta SQL en conexión '{conn_id}'...")

    dsn = f"{config['host']}/{config['service_name']}"
    conn = oracledb.connect(user=config['user'], password=config['password'], dsn=dsn, port=config.get('port', 1521))
    cursor = conn.cursor()
